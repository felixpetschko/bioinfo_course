...
In this exercise set, you will practice how to access, subset, and modify R objects. The topics include vectors, matrices, data frames, factors, logical conditions, sorting, and combining objects.
...
❓ **Question:** Without executing the code below, what is the length of vector `z`?
...
Use the `weight` and `height` columns to compute the body mass index (BMI) according to the following formula, and save it in the `info` data.frame, in a new column called `bmi` (i.e., `info[, "bmi"]` or `info$bmi`):

$$
BMI = \frac{\text{weight in kg}}{(\text{height in m})^2}
$$ 

The updated `info` data.frame should look like this:

```{r, include=TRUE, echo=FALSE}
info <- readRDS("~/locked/Data/bmi.rds")
info[,"bmi"] <- info[,"weight"] / info[,"height"]^2
print(info)
```

Save the updated `info` data.frame in a local directory (not "Data" directory, where you do not have writing access), in an RDS file called "info_w_bmi.rds".
...
What results do you get if you sum up the elements of `v1`? And for `v2` and `v3`? How would you interpret the results obtained for `v3`?
...
Build a vector of factors storing the information about which mice are wild type ("WT") or mutant ("MU") (use `rep`).

Add names to the vector elements to save the mouse identifiers (i.e., "M1", "M2", "M3", ...).

Apply the `table` function to the vector to count how many "WT" and "MU" mice there are.
