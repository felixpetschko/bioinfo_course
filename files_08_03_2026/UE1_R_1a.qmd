...
Save the length of the base and height of the rectangle depicted above into two numeric variables called, respectively, `base` and `height`.
...
Create a data.frame to store the info about the experiment. Precisely, we should have a row for each sample, and the following columns: cell line identifier (i.e., "CL1", "CL2", ...), lung-cancer subtype, treatment.
...
Load the "bmi.rds" file from the "Data" directory using `readRDS` (the full path is `~/locked/Data/bmi.rds`), and save it to a variable called `info`. This file contains information regarding the weight in kg and height in m of a group of people.
